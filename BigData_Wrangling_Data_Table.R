...
dt <- data.table(A = c(1, 2, NA, 4), B = c(NA, "x", "y", "z"))
dt

#checking for rows with NA
dt[is.na(A)] 
...
dt[, A := fifelse(is.na(A), 0, A)] 
dt[, B := fifelse(is.na(B), "missing", B)] 

## Imputing with descriptive stats(imputation)
dt[, A := fifelse(is.na(A), mean(A, na.rm = TRUE), A)]


## forward/backward Fill
dt[, A := nafill(A, type = "locf")] 
dt[, A := nafill(A, type = "nocb")]  


# Handling Duplicates ----
data_table <- data.table(A = c(1, 2, 2, 3, 3, 3), B = c("x", "y", "y", "y", "z", "z"), C = c("x", "y", "y", "z", "z", "z"))
data_table

data_table[duplicated(data_table)] 

## Removing duplicates
data_table_unique <- unique(data_table) 
data_table_unique
...
library(ggplot2)
set.seed(123)
data <- data.table(A = c(rnorm(20, mean = 50, sd = 10), 150, 160))  

boxplot(data$A, main = "Boxplot of A", col = "lightblue")
...
Q1 <- data[, quantile(A, 0.25)] 
Q3 <- data[, quantile(A, 0.75)] 

## calculating the IQR
IQR_value <- Q3 - Q1          

## generating class boundaries
lower_bound <- Q1 - 1.5 * IQR_value
upper_bound <- Q3 + 1.5 * IQR_value

# identifing outliers
outliers <- data[A < lower_bound | A > upper_bound]
outliers


# removing outliers
dt_clean <- data[A >= lower_bound & A <= upper_bound]
dt_clean

boxplot(dt_clean$A, main = "Boxplot of A", col = "lightblue")
